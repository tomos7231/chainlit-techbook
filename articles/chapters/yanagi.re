...
@<code>{on_audio_end}内で、WAVに変換した音声データをOpenAIのWhisper APIに送信します。今回は英会話を想定しているので、@<code>{language="en"}を指定しています。

=== システムプロンプトの設計

音声認識でテキスト化されたユーザーの発言は、LLMに送信されて応答が生成されます。その際に英会話パートナーの振る舞いを決めるのがシステムプロンプトです。
...
ユーザーにとって難しい返答になりすぎないよう、語彙のレベルや応答の長さを調整するよう指示しています。

=== 応答生成と音声合成

テキスト入力・音声入力のどちらであっても、最終的には@<code>{generate_response}関数で応答を生成します。
